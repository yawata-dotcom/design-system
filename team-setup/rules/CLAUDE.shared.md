...
## セッションの使い方（トークン節約・先回り通知）
利用者は会話の“重さ”や話題転換に気づきにくい。だからClaudeから返答の冒頭で先に知らせる。
- **話題が明確に変わったら** →「話題が変わったので新規チャット（`/clear`）がおすすめです」と必ず伝える。
- **会話が長く重くなってきたら** →「重くなってきたので `/compact` か新規チャットを」と伝える
  （目安。正確な使用量メーターは見えないので断定しない）。
- 使用量の確認は `/context`。原則 **「1つのセッション＝1つの話題」** に絞る（別の話は新規セッションで）。
